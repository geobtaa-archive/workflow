...
multiple_dict = { #dictionary to translate multivalue Dublin Core/GBL fields into GBLJson
    ...
    "Access Rights":["dct_accessRights_sm"]
    }
if not os.path.exists("json"): #create a folder to store the jsons if one does not already exist
    os.mkdir("json")

csvfile = open('arc-dec.csv', 'r') #opens the csv with the GBL data. Change this as needed
    ...
    for key,val in row.items():
        ...
        if key == "Bounding Box":
            ...
            if len(val) == 4: #takes care of bounding box values and calculates centroid
                ...
                east = val[2]
                north = val[3]
                centerlat = round((float(north)+float(south))/2,4)
                centerlong = round((float(east)+float(west))/2,4)
